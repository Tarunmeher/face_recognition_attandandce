            ...
            resizedDetections.forEach(async (detection) => {
                if (detection.descriptor) {
                    ...
                    if (bestMatch.label !== 'unknown') {
                        if (!attendeeList.includes(bestMatch.label)) {
                            attendeeList.push(bestMatch.label)
                            // document.getElementById('status').innerText = `${bestMatch.label} marked present`;
                            let length = $("#list tbody tr").length;
                            $("#list tbody").append('<tr><td>' + (length + 1) + '</td><td>' + bestMatch.label + '</td><td>' + new Date().toDateString() + '</td></tr>')
                            // Send attendance to server
                            await fetch('/api/attendance', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ name: bestMatch.label.split("-")[1].split(".")[0] })
                            });
                        }
                    }
                }
